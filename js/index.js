    ...
    document.addEventListener('keydown', (e)=>{
        ...
        switch(e.key){
            ...
            case 'i':
                ...
                break;
				case 's':
                    alerta.innerHTML = `PROCESO SUSPENDIDO`;
                    if(!pause && procesoActual != PROCESO_NULL){
						let process = procesosBloqueados.shift();
						process.bloqueado = 0
                        setProcesoEstado(process, estados.SUSPENDIDO, 'NULL');
                        memory.update(process.id, estados.SUSPENDIDO);
                        procesosSuspendidos.push(process);

						// Mostrar la alerta
						alerta.style.display = 'suspendido';
                ...
                break;
				case 'r':
                alerta.innerHTML = `PROCESO LISTO`;
                    if(!pause && procesosSuspendidos.length){
                        let process = procesosSuspendidos.shift();
						process.suspendido = 0;
						...
						// Mostrar la alerta
						alerta.style.display = 'ya no esta suspendido';
...
function sacarDeSuspendidos(){
	if(procesosBloqueados.length){
        let process = procesosSuspendidos.shift();
        process.suspendido = 0;
        setProcesoEstado(process, estados.LISTO, '-');
        procesosListos.push(process);
...
function actualizarMemoria(){
   ...
   memory.occupiedFrames.forEach(index=>{
        ...
        if(frame.state == estados.LISTO){
		 ...
		 else if(frame.state == estados.SUSPENDIDO){
            progressBar.className='suspendido';
        }
